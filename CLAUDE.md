...
### 정확도 (25-26 시즌 실시간 검증, 2025-12-05 기준)
| 구분 | 경기 수 | 정확 | 정확도 |
|------|---------|------|--------|
| **전체** | 322 | 232 | **72.05%** |
| 고신뢰(≥70%) | 129 (40.1%) | 112 | **86.82%** |
| 저신뢰(<70%) | 193 (59.9%) | 120 | **62.18%** |

*참고: 훈련 시점(22-25 시즌) 검증 정확도는 78.05%였으나, 실제 25-26 시즌 검증 결과 72.05%*
...
### 후행 지표 2: Return Penalty v1.0.0
| 항목 | 값 |
|------|-----|
| 버전 | 1.0.0 (2025-12-05) |
| 연구 근거 | bucketslab_1205 (선수 개인 모멘텀 연구) |
| 기본 페널티 | -4.6%p (7일+ 결장 후 복귀 시) |
| 공식 | `return_penalty = 4.6% × min(player_epm × 0.2, 1.0)` |
| 적용 조건 | EPM > 0, MPG ≥ 12, 결장일 ≥ 7일 |
| 적용 시점 | Injury Adjustment 후 적용 |

**연구 결과 (bucketslab_1205):**
| 구분 | 승률 | 차이 |
|------|------|------|
| 7일+ 결장 후 복귀 | 46.3% | -4.6%p |
| 일반 경기 | 50.9% | 기준 |

**적용 예시:**
- LeBron James (EPM 4.5) 14일 결장 후 복귀
- Return Penalty = 4.6% × (4.5 × 0.2) = 4.14%
- 홈팀 승률 -4.14%p 감소
...
### 25-26 시즌 실시간 검증 (2025-12-05, 322경기)
| 모델 | 전체 정확도 | 저신뢰(<70%) | 고신뢰(≥70%) |
|------|-------------|--------------|--------------|
| **V5.4** | **72.05%** | 62.18% | **86.82%** |
